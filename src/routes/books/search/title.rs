...
mod tests {
    ...
    async fn test_search_title_ok() {
        ...
        assert_eq!(
            resp.status(),
            200,
            "GET /books/search/title should return status 200"
        )
    }

    #[actix_web::test]
    async fn test_search_title_invalid() {
        let db_pool = db_utils::establish_pool_connection();
        ...
        assert_eq!(
            resp.status(),
            400,
            "GET /books/search/title should return status 400"
        )
    }
}
